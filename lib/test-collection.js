...
TestCollection.prototype.add = function (test) {
	...
	if (type !== 'test') {
		if (metadata.exclusive) {
			throw new Error('"only" can not be used with a ' + type + ' test');
		}
...
TestCollection.prototype._buildTestWithHooks = function (test) {
	...
	items.push.apply(items, beforeHooks);
	items.push(this._buildTest(test, context));
	items.push.apply(items, afterHooks);
